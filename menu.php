...
<?php	if(!empty($_GET['id'])){ ?>
		<?php
			extract($_GET); 
			$k = mysqli_query($con,"SELECT * FROM produk where id='$id'"); 
		...
		<div class="col-md-9">
			...
			<h3>Detail : <?php echo $data['nama'] ?></h3>
				...
				<?php $kat = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM kategori_produk where id='$data[kategori_produk_id]'"));  ?>
					...
					<p style="font-size:18px">Harga :<?php echo "Rp " .number_format($data['harga'], 3, ''. '.') ?></p>
					<p>
...
<?php }elseif(!empty($_GET['kategori'])){ ?>	
		...
		<?php
			extract($_GET); 
			$kat = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM kategori_produk where id='$kategori'")); 
		?>
		<div class="col-md-9">
			<div class="row">
			<div class="col-md-12">
			...
			<h3>Kategori : <?php echo $kat['nama'] ?></h3>
				<?php 
					$k = mysqli_query($con,"SELECT * FROM produk where kategori_produk_id='$kategori'");
				...
				<div class="col-md-4 content-menu">
					...
					<p style="font-size:18px">Harga :<?php echo number_format($data['harga'], 3, ''. '.') ?></p>
					<p>
...
<?php }else{ ?>	
			...
			<div class="col-md-12">
			<hr>
			<center> <h3 >Daftar Semua Produk</h3> </center>
				<?php 
					$k = mysqli_query($con,"SELECT * FROM produk");
					while($data = mysqli_fetch_array($k)){
				?>
				<div class="col-md-4 menu_utama">
					<a href="<?php echo $url; ?>menu.php?id=<?php echo $data['id'] ?>">
						<img src="<?php echo $url; ?>uploads/<?php echo $data['gambar'] ?>" width="100%">
						<center><h4><?php echo $data['nama'] ?></h4>
					</a>
					 <p style="font-size:18px">Harga :<?php echo number_format($data['harga'], 3, ''. '.') ?></p>
					<p>
						<a href="<?php echo $url; ?>menu.php?id=<?php echo $data['id'] ?>" class="btn btn-success btn-sm" href="#" role="button">Lihat Detail</a>
