<?php 
	...
	if(!empty($_POST)){
		...
		$q = mysqli_query($con,"SELECT * FROM `user` WHERE email='$email' AND password='$password' AND status='user'");
		if($q){
...
?> 
		...
		<div class="col-md-9">
			...
			$q = mysqli_query($con,"insert into kontak Values(NULL,'$nama','$email','$subjek','$pesan')");
				if($q){  
			?>
