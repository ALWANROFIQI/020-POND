...
?>
		...
		<?php
			if(!empty($_POST['form-order'])){
				...
				$kt = mysqli_fetch_object(mysqli_query($con,"SELECT *FROM kota where id='$kota'"));
				if($difference->days >= 0){
					
					$q = mysqli_query($con,"insert into pesanan Values(NULL,'$tanggal_pesan','$tanggal_digunakan','$_SESSION[iam_user]','$nama','$alamat','$kt->nama','$kt->ongkir','$telephone','0','belum lunas')");
					if($q){

						$last = mysqli_fetch_array(mysqli_query($con,"SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							$product = mysqli_fetch_array(mysqli_query($con,"select * from produk WHERE id='$id'"));
							if(!empty($product)){

								$ins = mysqli_query($con,"insert into detail_pesanan Values(NULL,'$product[id]','$qty','$last[id]')");
				...
				<hr>
				 <form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kota" required class="form-control">
							...
							?>
							<option value="<?php echo $kp['id']; ?>"><?php echo $kp['nama'] ?> - <?php echo "Rp ".number_format($kp['ongkir'], 3,'.', '.'); ?></option>
								<?php } ?>
						</select><br>
	...
	if(!empty($product)){
		...
		?>
				<tr class="barang-shop">
					...
					<div class="price"><?php echo  "Rp ".number_format($product['harga'], 3, '.', '.') ?></div>
					</div></td>
					<td>
						<?php echo $qty ?> pcs
					</td>
					<td class="price"><?php echo number_format($t, 3, '.', '.') ?></td>
